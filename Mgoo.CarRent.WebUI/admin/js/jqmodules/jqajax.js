...
layui.define(['jquery', 'layer'], function (exports) {
    ...
    jqajax.prototype.ajax = function (options) {
        ...
        if (options.action == "list") {
            if (options.url.indexOf('&p=') < 0) {
                options.url += "&p=1";
            }
            if (options.url.indexOf('&pagesize=') < 0) {
                options.url += "&pagesize="+top.global[options.dataName].options.pageNum;
            }
        } 
        //变为大写
        ...
        $.ajax({
            ...
            beforeSend: function (XMLHttpRequest) {
                ...
                if (ui) {
                    ui = JSON.parse(ui) || {};
                    XMLHttpRequest.setRequestHeader("Token", ui.token);
                } 
            },
            error: function (XMLHttpRequest, status, thrownError) {
                layer.msg('The network is busy, please try again later...', { icon: 2 });
            },
    ...
    /**
     *@todo ajax成功后执行方法
     *@param ret object 服务端返回的信息 ret={code:0,message:'',result:{url:'baidu.com'}}
     *@param options object 拼装好的参数对象
     **/
    ...
    jqajax.prototype.complete = function (ret, options) {
        ...
        if (0 == ret.code) {
            ...
            if (options.action == "list") {
                options.data = { list: result.list };//ret.data;
                options.pages = result.pages;
                options.pageTotal = result.total;
            }
            ...
            if (!result.url) { // || (undefined == result.url) || (null == result.url) || ("" == result.url)
               
                if (index) {
                    parent.layer.close(index);
                    if (ret.message) {
                        parent.layer.msg(ret.message, { icon: 6 });
                    }
                } else {
                    layer.closeAll();
                    if (ret.message) {
                        layer.msg(ret.message, { icon: 6 });
                    }
                }
            } else {
                if ('reload' == result.url) {
                    if (index) {
                        parent.layer.close(index);
        ...
        if (options.confirm) {
            ...
            if (options.title) {
                ...
                if (obj) {
                    ...
                    title = 'confirm<span class="color-commred">' + title + '</span> operation';
                }
            }
            layer.confirm(title, { icon: 3, title: 'prompt', btn: ['Ok', 'NO'] }, function (index) {
