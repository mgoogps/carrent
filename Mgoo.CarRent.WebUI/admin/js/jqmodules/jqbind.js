    ...
    jqbind.prototype.modal = function (obj) {
        ...
        var iframeOpts = {
            ...
            success: function (layero, index) {
                ...
                if (options.pars) {
                    iframeWin.load(options.pars);
                }
            }
        }
        if (options.scroll == false || options.scroll == undefined) {
            iframeOpts.content = [iframeOpts.content,'no'];
        } 
        if (!options.area) {
            var l = layer.open(iframeOpts);
            layer.full(l);
        } else {
            if (_area.length == 1) {
                _area = "'" + _area[0] + "'";
            }
            iframeOpts.area = _area;
            var l = layer.open(iframeOpts);
        }
    }
