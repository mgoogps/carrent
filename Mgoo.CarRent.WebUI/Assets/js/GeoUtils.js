...
(function() { 
    ...
    GeoUtils.isPointInRectGoogle = function (point, SouthWest, NorthEast) {
        //检查类型是否正确
        //if (!(point instanceof BMap.Point) || 
        //    !(bounds instanceof BMap.Bounds)) {
        //    return false;
        //}
        var sw = SouthWest; //西南脚点
        var ne = NorthEast; //东北脚点
        return (point.lng >= sw.lng && point.lng <= ne.lng && point.lat >= sw.lat && point.lat <= ne.lat);
    }
     ...
     */
    GeoUtils.isPointInPolygon = function(point, polygon,sw,ne){
        //检查类型
        //if(!(point instanceof BMap.Point) ||
        ...
        if (!this.isPointInRectGoogle(point, sw,ne)) {
            return false;
        }

        var ptsArray = polygon.getPath().getArray();//获取多边形点
        var pts = [];
